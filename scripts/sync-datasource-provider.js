...
function syncDatasourceProvider() {
  ...
  syncMigrationLock(provider);
}

/**
 * Prisma requires migration_lock.toml provider to match schema datasource provider (P3019 if not).
 * Migration SQL in this repo targets PostgreSQL; SQLite dev uses `db push` instead of migrate deploy.
 */
function syncMigrationLock(provider) {
...
module.exports = { syncDatasourceProvider, resolveProvider, syncMigrationLock };
